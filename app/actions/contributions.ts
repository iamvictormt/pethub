...
export async function createPixContribution(
  ...
  console.log('Creating PIX contribution for amount:', amountInCents);
  ...
  console.log('Auth check - User:', user?.id || 'anonymous');
  console.log('Auth check - Email:', user?.email || 'none');
  console.log('Auth check - Error:', authError?.message || 'none');
  ...
  try {
    ...
    console.log('Mercado Pago payment created:', payment.id);
    ...
    if (error) {
      console.error('Supabase client insert failed:', {
        code: error.code,
      ...
      console.log('Trying fallback with direct REST API call...');
      ...
      console.log('Request headers (keys only):', Object.keys(headers));
      console.log('Making request to:', `${supabaseUrl}/rest/v1/contributions`);
      ...
      if (!response.ok) {
        ...
        console.error('REST API insert failed:', {
      ...
      console.log('Created contribution via REST API:', restContribution.id);
    ...
    console.error('Error in createPixContribution:', error);
    throw error;
  }
}
...
export async function checkPaymentStatus(contributionId: string) {
  ...
  if (isExpired && contribution.status === 'pending') {
    ...
    if (updateError) {
      console.error('Failed to mark contribution as expired:', updateError);
    }
...
export async function markContributionAsCompleted(contributionId: string, transactionId?: string) {
  ...
  if (error) {
    console.error('Failed to mark contribution as completed:', error);
